...
void finalOutput(int problemID, double parameters[], double ans,
	...
	case PROBLEM3:
		printf("ans: x= %.18e |f(x)|= %.10e iteration= %2d\n", ans, absfval, iteration);
		printf("ans: y= %.18e", 1 * ans - 0.25);
		break;
	case PROBLEM4:
		printf("ans: x= %.18e |f(x)|= %.10e iteration= %2d\n", ans, absfval, iteration);
		break;
	case PROBLEM5:
		printf("ans: x= %.4f |f(x)|= %.10e iteration= %2d\n", ans, absfval, iteration);
		break;
		// 수식(문제)이 추가될 때마다 이곳에 case 문을 추가하여
...
int main(void) {
	...
	for (int t = 0; t < TN; t++) {
		...
		case SECANT:
			...
			//printf("%lf %lf %d %lf", x0, x1, checkConvergence, root);
			scanf("%d %lf %lf %d", &maxIter, &delta, &epsilon, &showSteps);
...
void Secant(
	...
	for (iter = 0; iter <= maxIter; iter++) {
		...
		x0 = x1;
		x1 = x1 - (_F(x1, p[0], p[1], p[2])*(1 / _FP(x1, p[0], p[1], p[2])));
	}
	// 최종 출력을 저장합니다
	*iteration = iter;	// 총 반복 회수
	*ans = x1;			// 구한 root 근사값(정확할 수도 있음)
...
void Bisection(
	...
	for (iter = 0; iter <= maxIter; iter++) {
		x1 = (L + R) / 2;
		//printf("L:%f R:%f x1:%f", L, R, x1);
		//printf("%f %f", L, R);
		if (showSteps == OUTPUT_STEPS) {
			// 중간 단계를 표시하는 코드
			if (iter == 0 || chkConvergence != CONVERGENCE_CHECK) {
				CC = (L + R) / 2;
				if (_F(L, p[0], p[1], p[2])*_F(x1, p[0], p[1], p[2]) < 0) R = CC;
				else L = CC;
				printf("%2d  %.18e  %.2e\n", iter, x1, fabs(_F(x1, p[0], p[1], p[2])));
			}
			else if (chkConvergence == CONVERGENCE_CHECK) {
