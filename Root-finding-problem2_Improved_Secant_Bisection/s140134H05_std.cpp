...
double genRandN_NewtonRaphson(double U, int si) {
	double x0 = 0.3;
	double x1 = 0.6;	// just for initial buid without coding
...
double genRandN_Secant(double U, int si) {
	double x0, x1;		// these variables are for initial build without coding
	...
	for (iter = 0; iter < maxIter; iter++)
	{
		...
		double xk = x1 - fabs(interpolationF(x1, U)) *	(x1 - x0) /	(fabs(interpolationF(x1, U)) - fabs(interpolationF(x0, U)));
		if (xk < 0) xk = -xk;
		else if (xk > 1) xk = 0.9;
		x0 = x1;
		x1 = xk;
...
double genRandN_Improved(double U, int si) {
	...
	for (iter = 0; iter < maxIter; iter++)
	{
		if (flag == 0)
		{
			...
			if (fabs(interpolationF(x1, U)) < delta) break;
			if (iter != 0 && fabs(R - L) < epsilon) break;
			x0 = x1;
			x1 = CC;
			if (iter >= (maxIter / 2))
			{
